...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ 
  ...
  const handleDownloadAll = async () => {
    ...
    if (analysis) {
      ...
      if (analysis.regionInfo) {
          sourceReport += `\n[REGION OF ORIGIN]\nRegion: ${analysis.regionInfo.region}\nConfidence: ${analysis.regionInfo.confidence}\nReasoning: ${analysis.regionInfo.reasoning}\n`;
      }
      if (analysis.locationData) {
          sourceReport += `\n[LOCATION DATA]\nLocation: ${analysis.locationData.title}\nGoogle Maps: ${analysis.locationData.uri}\nDetails: ${analysis.locationData.description}\n`;
      }
    ...
    <div className="w-full mx-auto animate-fade-in space-y-8">
      ...
      {isFullscreen && restoredImageUrl && (
        <div className="fixed inset-0 z-[100] bg-heritage-950/95 backdrop-blur-lg flex flex-col animate-fade-in">
            ...
            <div className="h-20 flex items-center justify-center gap-6 bg-heritage-900 border-t border-royal-900/30 relative z-40">
        ...
        <div className="lg:col-span-7 space-y-8">
           ...
           {analysis?.sourceInfo && (
            <AnimatedBorder seed={505} type={kolamType}>
              <div className="bg-[#1a0b05] p-1 shadow-2xl rounded-xl transform transition-transform hover:scale-[1.01]">
                <div 
                    className={`rounded-lg overflow-hidden relative transition-all duration-500 bg-[#2a1205] border ${isSourceIdentified ? 'border-royal-500/40 shadow-[0_0_20px_rgba(251,191,36,0.1)]' : 'border-royal-900/30'}`}
                >
                    <div className="px-6 py-4 border-b border-royal-900/20 flex items-center gap-3 bg-heritage-900/30">
                        ...
                        <h2 className={`text-sm font-serif font-bold uppercase tracking-[0.2em] ${isSourceIdentified ? 'text-royal-400' : 'text-royal-500/70'}`}>Source Detected</h2>
                    </div>
                    
                    <div className="p-6 md:p-8">
                        <h3 className={`text-2xl md:text-3xl font-serif font-bold leading-tight mb-2 ${isSourceIdentified ? 'text-royal-300' : 'text-amber-200/70'}`}>
                        ...
                        <p className="font-serif italic text-parchment-200/80 leading-relaxed border-l-2 border-royal-700/30 pl-4 mb-6">
                            {analysis.sourceInfo.briefExplanation}
                        </p>

                        {/* Region Info Section */}
                        {analysis.regionInfo && (
                            <div className="mt-6 pt-6 border-t border-royal-900/30 flex flex-col sm:flex-row gap-6">
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-2">
                                       <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-royal-500" viewBox="0 0 20 20" fill="currentColor">
                                         <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                                       </svg>
                                       <span className="text-xs font-bold uppercase tracking-widest text-royal-600">Origin</span>
                                    </div>
                                    <p className="text-lg font-serif text-parchment-100">{analysis.regionInfo.region}</p>
                                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full mt-1 inline-block ${analysis.regionInfo.confidence.toLowerCase().includes('high') ? 'bg-green-900/30 text-green-400' : 'bg-royal-900/30 text-royal-400'}`}>
                                        {analysis.regionInfo.confidence} Confidence
                                    </span>
                                    ...
                                    {analysis.locationData && (
                                        <div className="mt-4">
                                            <a 
                                                href={analysis.locationData.uri} 
                                                target="_blank" 
                                                rel="noopener noreferrer"
                                                className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-royal-400 border border-royal-700/50 px-3 py-1.5 rounded hover:bg-royal-900/40 transition-colors"
                                            >
                                ...
                                <div className="flex-[2] border-l border-royal-900/30 pl-6 sm:pl-6 border-t sm:border-t-0 pt-4 sm:pt-0">
                                     <p className="text-sm text-parchment-300/80 italic mb-2">"{analysis.regionInfo.reasoning}"</p>
                                     {analysis.locationData && (
                                         <p className="text-xs text-royal-500/60 font-mono mt-2 border-t border-royal-900/20 pt-2">
                                            📍 {analysis.locationData.description}
                                         </p>
                                     )}
                                </div>
                            </div>
